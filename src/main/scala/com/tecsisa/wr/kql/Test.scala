...
object Test {
  ...
  object Parser {
    ...
    val ForbiddenIndexChars = NamedFunction(
        !"./\\*?\"<>| ,()".contains(_: Char), "ForbiddenIndexChars")
    ...
    val queryClause =
      P(space ~ fieldP.! ~ space ~ operatorP.! ~ space ~ valueP.!).map(
    ...
    val limitSection = P(space ~ `with` ~ space ~ limit ~ space ~ integral.!)
      .map(_.toInt)
      .map(Limit)

    val expr =
      P(space ~ searchSection ~ space ~ indexSection ~ limitSection.? ~ querySection ~ space ~ End).map {
